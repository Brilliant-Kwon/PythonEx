def define_list():
    ...
    lst2 = []  # 리스트리터럴로 생성 : 일반적
    lst3 = [1, 2, "Python"]
    lst4 = list("Python")  # 다른 시퀀스 객체를 리스트로 생성
...
def list_methods():
    ...
    lst_copy.sort(reverse=True)  # 기본 소트 : 내림차순 정렬
    print("desc lst_copy:", lst_copy)
    lst_copy.sort(key=int)
    print("int key sort:", lst_copy)
    lst_copy.sort(key=str)
    print("int str sort:", lst_copy)  # 문자열 기준으로 정렬
